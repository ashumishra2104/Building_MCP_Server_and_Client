...
- **`src/helper.py`** — All OpenAI calls. Uses `gpt-4.1` for resume tailoring and cover letter generation (high-quality, strict rules), `gpt-4o-mini` for lightweight tasks (summarise, extract post signal, generate search titles). Key functions:
  ...
  - `generate_poster_dm(resume_text, job_title, company, poster_name, candidate_name, candidate_website, candidate_github)` — generates a ready-to-send LinkedIn DM to a job poster. Uses `gpt-4o-mini` to write 2-3 fit lines; the salutation, opener, closing, and signature are deterministic template. Salutation: "Hi {first_name}," if poster_name is set, else "Hi,".
...
- **`src/ui_components.py`** — All card renderers (`render_linkedin_card`, `render_naukri_card`, `render_indeed_card`, `render_linkedin_post_card`) and the shared `_actions_block` helper that renders the "⚡ Actions" expander. `_actions_block` accepts optional `poster_name` and `poster_url` kwargs; when `poster_url` is set it shows 4 tabs (Tailor Resume / Cover Letter / Full JD / 💬 DM Poster), otherwise 3. The DM Poster tab calls `generate_poster_dm()` and shows the result in a copyable text area with a link to the poster's LinkedIn profile. `_tailor_tab()` auto-fetches approved ATS keywords from DB. `JOB_CARD_CSS` is injected by every page that renders cards. `render_linkedin_card` shows a "👤 Posted by" row (from `posterFullName`/`posterProfileUrl`) and a "💬 Message Poster" footer button; falls back to "Poster info not available" when absent.
...
| `user_profiles` | `user_email` (PK/upsert key), `profile_name`, `resume_text`, `candidate_name`, `candidate_email`, `candidate_phone`, `raw_pdf_name`, `candidate_website`, `candidate_github`, `is_active`, `updated_at` |
